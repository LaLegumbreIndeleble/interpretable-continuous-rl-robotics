"""
DEAP Multi-Output GP Controller — BipedalWalker-v3
====================================================
Setup & Run:
    uv init deap_demo
    cd deap_demo
    uv add deap numpy "gymnasium[box2d]"
    uv run bipedal_gp.py

Anti-stagnation improvements over v1:
  1. Richer operator set  (15 operators vs 8)
  2. Two constant pools: small [-1,1] and large [-3,3]
  3. Higher tree height cap  (6 vs 4)
  4. Island model with ring migration every N gens
  5. Mixed mutation: subtree + node replacement + shrink
  6. Stagnation detector per island → injects fresh immigrants
  7. Single-best elitism per generation
"""
...
def make_individual():
    return creator.Individual([toolbox.tree() for _ in range(N_ACTIONS)])
...
def print_individual(ind, label=""):
    reward = ind.fitness.values[0]
    print(f"\n{'─'*62}")
    print(f"  {label}  |  reward = {reward:.2f}")
    print(f"{'─'*62}")
    for i, tree in enumerate(ind):
        print(f"  action[{i}] ({ACTION_NAMES[i]:8s}) = {tree}")
    print(f"{'─'*62}")
...
def main():
    ...
    for gen in range(1, N_GEN + 1):
        ...
        for idx, isle in enumerate(islands):
            ...
            # stagnation check
            cur_best = max(isle, key=lambda ind: ind.fitness.values[0]).fitness.values[
                0
            ]
    ...
    # pickle — full individual, loadable by run_best.py
    with open(os.path.join(BEST_MODEL_DIR, "best_individual.pkl"), "wb") as f:
        pickle.dump(best, f)

    # human-readable JSON — tree expressions + fitness
    meta = {
        "fitness": best.fitness.values[0],
        "trees": {ACTION_NAMES[i]: str(best[i]) for i in range(N_ACTIONS)},
    }
