...
Architecture: 9 trees per individual
  ...
  trees 4-7  (var)   → raw variance proposals; softplus applied externally → > 0
  tree  8    (critic) → TD(0) baseline V(s), gates actions at runtime

Fitness = mean episode reward (identical to v2).

vs v2: probe step now samples from N(tanh(mu), sqrt(softplus(var))) instead
of using raw clipped mu.  Critic gating is unchanged.  The stochastic
exploration comes from the evolved variance trees rather than a fixed schedule.

    mu         = tanh(mu_tree(*obs))         ← bounded mean
    sigma      = sqrt(softplus(var_tree(*obs)))   ← positive std
    a_samp ~ N(mu, sigma), clipped to [-1, 1]    ← probe action
    v          = critic(*obs)
    v_next     = critic(*obs_after_probe)
    advantage  = reward_probe + 0.99*v_next - v
    gate       = sigmoid(advantage)
    action     = clip(a_samp * gate, -1, 1)       ← committed action

Run:
    uv run deap_demo/bipedal_gp_a2c.py
"""
...
def evaluate(individual):
    ...
    for ep in range(N_EPISODES):
        ...
        for _ in range(MAX_STEPS):
            ...
            action = np.clip(
                [a_samp[i] * gate for i in range(N_ACTIONS)],
                -1.0, 1.0,
            ).astype(np.float32)
